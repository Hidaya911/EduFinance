...
def student_list(request):
    ...
    # AJAX request:
    # return only the results instead of entire page.
...
def student_delete(
    ...
    # Never allow state-changing actions through GET.
    if request.method != "POST":
        raise Http404()
...
def get_guardian(guardian_id):
    try:
        object_id = ObjectId(guardian_id)

    except (InvalidId, TypeError):
        raise Http404("Invalid guardian ID.")
...
def guardian_edit(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Replace existing relationships safely.
            StudentGuardian.objects.filter(
                guardian_id=str(
                    guardian.pk
                )
